...
$query = "
    SELECT idx, created_at, date_format(created_at, \"%m-%d\") as DF,
        (MAX(IF(board_number=6, data1, NULL)) - MIN(IF(board_number=6, data1, NULL)) ) as daily_5building
    FROM upa.raw_data
    WHERE created_at >= \"2023-11-02\" and created_at < current_date() and address = '3213' and board_type = 35   
    group by DF
    ORDER BY idx asc;
    "; 
...
foreach ($rows as $k => $v) {
    array_push($daily_1building_arr, array($k, ($v['daily_5building'])));
...
$response['pay_load']['dataset'] = array('daily_5building'=>$daily_1building, );
