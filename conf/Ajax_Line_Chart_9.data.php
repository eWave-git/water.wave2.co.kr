...
$query = "
    SELECT idx, created_at, date_format(created_at, \"%m-%d\") as DF,
        (MAX(IF(board_number=5, data1, NULL)) - MIN(IF(board_number=5, data1, NULL)) ) as daily_4building
    FROM upa.raw_data
    WHERE created_at >= \"2023-11-01\" and created_at < current_date() and address = '3213' and board_type = 35   
    group by DF
    ORDER BY idx asc;
    "; 
